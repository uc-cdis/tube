...
RUN pip install pip==9.0.3 && pip install --upgrade pip && pip install --upgrade setuptools &&\
    wget ${SQOOP_INSTALLATION_URL} &&\
    mkdir -p $SQOOP_HOME &&\
    ...
    mv postgresql-42.2.4.jar $SQOOP_HOME/lib/ &&\
    wget ${HADOOP_INSTALLATION_URL} &&\
    mkdir -p $HADOOP_HOME &&\
    tar -xvf hadoop-${HADOOP_VERSION}.tar.gz -C ${HADOOP_HOME} --strip-components 1 &&\
    rm hadoop-${HADOOP_VERSION}.tar.gz &&\
    wget ${ES_HADOOP_INSTALLATION_URL} &&\
...
ENV HADOOP_CONF_DIR=$HADOOP_HOME/etc/hadoop \
    ...
    PATH="${PATH}:${SQOOP_HOME}/bin:${HADOOP_HOME}/sbin:$HADOOP_HOME/bin:${JAVA_HOME}/bin"

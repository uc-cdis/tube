...
class Translator(BaseTranslator):
    ...
    def merge_auth_root(self, root):
        ...
        while child is not None:
            ...
            df = df.join(self.translate_edge(edge_tbl)).map(
                lambda x: (x[1][1], x[1][0])
            )
            tbl_name = child.tbl_name
            df = df.join(self.translate_table(tbl_name, props=child_props)).mapValues(
                lambda x: merge_and_fill_empty_props(x, child_props)
            )
    ...
    def merge_roots_to_children(self):
        ...
        for root in self.parser.roots:
            df = (
                self.translate_table(root.tbl_name, props=root.props)
                if root.root_child is None
                else self.merge_auth_root(root)
            )
            for child in root.children:
                edge_tbl = child.parents[root.name]
                tmp_df = df.join(self.translate_edge(edge_tbl))
                tmp_df = tmp_df.map(lambda x: (x[1][1], x[1][0])).mapValues(

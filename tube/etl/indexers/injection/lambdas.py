...
def get_frame_zero(props):
    return tuple([get_single_frame_zero_by_func(p.fn, p.name) for p in props])
...
def get_normal_frame(props):
    ...
    :return:
    """
    return lambda x: tuple([(p.fn, p.name, get_single_frame_value(p.fn, x.get(p.name)))
                            for p in props])
...
def get_props_to_tuple(x, props):
    get_props = []
    for p in props:
        get_props.append(tuple([p.fn, p.name, get_single_frame_value(p.fn, x.get(p.name))]))

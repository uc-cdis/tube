...
def configure_core_site():
    ...
    root = tree.getroot()
    root.append(
        create_property("hadoop.tmp.dir", "{}/hdfs/tmp".format(config.HADOOP_HOME))
    )
...
def configure_yarn_site():
    ...
    root.append(
        create_property(
            ...
            "{}:8030".format(config.HADOOP_HOST),
        )
    )
    root.append(
        create_property(
            "yarn.resourcemanager.resource-tracker.address",
            "{}:8031".format(config.HADOOP_HOST),
        )
    )
    root.append(
        create_property(
            "yarn.resourcemanager.address", "{}:8032".format(config.HADOOP_HOST)
        )
    )
    tree.write(core_site_path)
